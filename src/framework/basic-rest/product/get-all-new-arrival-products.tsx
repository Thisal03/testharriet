...
export const fetchNewArrivalProducts = async (
  page: number = 1,
  perPage: number = 50,
  options?: { sortOption?: string; minPrice?: number; maxPrice?: number }
): Promise<PaginatedSaleProduct> => {
  try {
    let url = `wp-json/wc/v3/products?status=publish&page=${page}&per_page=${perPage}`;
...
export const useNewArrivalProductsQuery = (options: {
  limit: number;
  ...
  return useInfiniteQuery<PaginatedSaleProduct, Error>({
    queryKey: ["new_arrival_products", options],
    queryFn: async ({ pageParam = 1 }) =>
      fetchNewArrivalProducts(pageParam as number, options.limit, options),
