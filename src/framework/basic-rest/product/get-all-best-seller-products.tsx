...
export const fetchBestSellerProducts = async (
  ...
  try {
    let url = `wp-json/wc/v3/products?status=publish&orderby=popularity&stock_status=instock&page=${page}&per_page=${perPage}`;
...
export const useBestSellerProductsQuery = (options: {
  ...
  return useInfiniteQuery<PaginatedSaleProduct, Error>({
    queryKey: ["best_seller_products", options],
    queryFn: async ({ pageParam = 1 }) =>
      fetchBestSellerProducts(pageParam as number, options.limit, options),
